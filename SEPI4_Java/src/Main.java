...
public class Main {
	...
	public static void main(String[] args) {
		...
		//try {
			...
			stopwatch.start();
			/*serialConnection.write("Q".getBytes());
			serialConnection.read();
	...
	public static int[] analyzeData(int[][] trackData) {
		int raceCounter = 0;
		int[] raceData= new int[700];
		...
		for (int i = 1; i < speedArr2.length; i++) {
			speedArr2[i] = speedArr2[i-1]+0.3*(speedArr[i]-speedArr2[i-1]);
			//System.out.println(speedArr2[i]);
		}
		raceData[0] = 0;
		for (int i = 0; i < speedArr.length; i++) {
			raceData[raceCounter+1]=(int) (speedArr2[i]*100);
